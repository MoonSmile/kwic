 ...
 */
  ...
  //1.6
  private int[] shift_circular_shifts_;
 ...
 */
  ...
  private int[] shift_alphabetized_;
  private HashMap<Integer, Integer> line_and_length;
 ...
 */
  ...
  public void circularShift(){
    ...
    shift_chars_ = new char[1000];
    shift_circular_shifts_ = new int[256];
    ...
    for(int i = 0; i < line_index_.length; i++){
      ...
      int line_length=line_end-line_index_[i];
      System.out.println("current line length:"+line_length);
      ...
      for(int j = line_index_[i]; j < line_end; j++){        
          ...
          if(j==line_index_[i]){
        	  System.arraycopy(chars_, j, shift_chars_, shift_circular_shifts_[shift_count], line_length);
          }
          else{
              System.arraycopy(chars_, j+1, shift_chars_, shift_circular_shifts_[shift_count], line_end-j-1);
              shift_chars_[shift_circular_shifts_[shift_count]+line_end-j-1]=' ';
              System.arraycopy(chars_, line_index_[i], shift_chars_, shift_circular_shifts_[shift_count]+line_end-j, j-line_index_[i]);        
          }
          shift_length+=line_length;
          shift_circular_shifts_[shift_count+1]=shift_length;
    ...
    if(shift_length!=shift_chars_.length)
    {
    	System.out.println("print");
    	char[] shift_chars_tmp = new char[shift_length];
    	System.arraycopy(shift_chars_, 0, shift_chars_tmp, 0, shift_length);
    	shift_chars_ = shift_chars_tmp;
    	
    }
        // set the columns size of shift matrix to the real number of shifts
    if(shift_count != circular_shifts_[0].length){
    	int[] shift_tmp = new int[shift_count];
    	System.arraycopy(shift_circular_shifts_, 0, shift_tmp, 0, shift_count);
    	shift_circular_shifts_ = shift_tmp;
    	System.out.println(shift_circular_shifts_);
 ...
 */
  ...
  public void alphabetizing(){
    ...
    shift_alphabetized_ = new int[shift_circular_shifts_.length];
    System.out.println("shift_circular_shifts_.length:"+shift_circular_shifts_.length);
    ...
    for(int i = 0; i < shift_alphabetized_.length; i++){
  ...
  public void shift_alphabetizing(){
	    
      // initialize the alphabetized matrix
  shift_alphabetized_ = new int[shift_circular_shifts_.length];
//  System.out.println("shift_circular_shifts_.length:"+shift_circular_shifts_.length);
  line_and_length = new HashMap<Integer, Integer>();
  ...
  int mid = 0;
      ...
      // process the circular shifts
  for(int i = 0; i < shift_circular_shifts_.length; i++){
	int line_start=shift_circular_shifts_[i]; 
	int line_end=(i==shift_alphabetized_.length-1)?shift_chars_.length:shift_circular_shifts_[i+1];
    char[] current_shift = new char[line_end - line_start];
    line_and_length.put(line_start, line_end - line_start);
    System.arraycopy(shift_chars_, line_start, current_shift, 0, line_end - line_start);
    ...
    while(low <= high){
      ...
      mid = (low + high) / 2;
          ...
          // the index of original mid line
      int mid_line_number = shift_alphabetized_[mid];
      int mid_line_length=line_and_length.get(mid_line_number);
      char[] mid_line = new char[mid_line_length];
    System.arraycopy(shift_chars_, mid_line_number, mid_line, 0, mid_line_length);
      ...
      default: // i-th line equal
        ...
        // copy the upper part of alphabetized arrays
    System.arraycopy(shift_alphabetized_, low, shift_alphabetized_, low + 1, alphabetized_count - low);
    
        // insert the i-th shifted line
    shift_alphabetized_[low] = line_start;
 ...
 */
  ...
  public void filter()
  {
	  int lastone=shift_circular_shifts_.length;
	  for(int i=0;i<lastone;i++)
	  {
		  char head_char = shift_chars_[shift_circular_shifts_[i]];
		  if (head_char>='0' && head_char<='9') {
			  int current_end=(i==lastone-1)?shift_chars_.length:shift_circular_shifts_[i+1];
			  int current_remove_length=current_end-shift_circular_shifts_[i];
			  char[] tmp = new char[shift_chars_.length-current_remove_length];
			  
			  if(i!=0)
			  {
				  System.arraycopy(shift_chars_, 0, tmp, 0, shift_circular_shifts_[i]);
			  }
			  if(i!=lastone-1)
			  System.arraycopy(shift_chars_, shift_circular_shifts_[i+1], tmp,shift_circular_shifts_[i+1]-current_remove_length, shift_chars_.length-shift_circular_shifts_[i+1]);
			  shift_chars_=tmp;
			  for(int j=i;j<lastone-1;j++){
				  shift_circular_shifts_[j]=shift_circular_shifts_[j+1]-current_remove_length;
			  }
			  lastone--;
			
		}
	  }
      int[] tmp = new int[lastone];
      System.arraycopy(shift_circular_shifts_, 0, tmp, 0, lastone);
      shift_circular_shifts_ = tmp;
	  
  }
  public void output(){
	  System.out.println("shift_alphabetized_.length:"+shift_alphabetized_.length);
    for(int i = 0; i < shift_alphabetized_.length; i++){
      int shift_start = shift_alphabetized_[i];
      
      int line_end = line_and_length.get(shift_start);
        for(int j = shift_start; j < shift_start+line_end; j++)
