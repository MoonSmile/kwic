 ...
 */
  ...
  public void update(Observable observable, Object arg){
    ...
    case LineStorageChangeEvent.DELETE:
    	...
    	for(int i = 0; i < line.length; i++){
    		String line_rep = line[i];
    		for(int j = 1; j < line.length; j++){
    			line_rep+=" "+line[(i+j)%line.length];
    		}
    		for(int k=0;k<shifts_.getLineCount();k++)
    		{
    			if(shifts_.getLineAsString(k).equals(line_rep))
    			{
    				shifts_.deleteLine(k);
    				break;
    			}
    		}
         }
